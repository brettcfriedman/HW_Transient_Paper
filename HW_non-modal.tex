...
Nevertheless, it is well-known in the hydrodynamics community that normal mode analysis fails at predicting turbulent onset for a large class of instabilities that are called subcritical.
Subcritical turbulence is certainly possible in plasmas, and several specific cases have been studied in the past~\cite{camargo1998,camporeale2010,schekochihin2012}. 
For the most part, however, normal mode analysis dominates plasma turbulence prediction.

Since the hydrodynamics community has long studied subcritical turbulence, it has much to offer in terms of understanding the phenomenon. Most importantly,
...
systems. However, to our knowledge, they have not been used to predict onset criteria in subcritical systems (e.g. the transition Reynold's number in pipe flows).
...
In nonlinear simulations, the turbulence can laminarize due to the build-up of $k_y=0$ density and potential structures~\cite{biskamp1995}. To prevent this, we subtract out the $k_y=0$
$n$ and $\phi$ components in the simulations, which is the equivalent of adding source terms to Eqs.~\ref{n_eq} and~\ref{phi_eq}~\cite{friedman2012b}.
...
\begin{figure}
\centerline{\includegraphics[width=0.4\textwidth]{gamma_max_vs_alpha}}
...
\begin{figure}
\centerline{\includegraphics[width=0.45\textwidth]{gamma_max_with_nm}}
